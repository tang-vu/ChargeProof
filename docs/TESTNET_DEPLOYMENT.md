...
Create dedicated local burners once, if no ignored environment files exist yet:
...
The creation command refuses to overwrite `.env` or `apps/web/.env.local`, writes keys only to those
...
Fund only the public deployer address printed by `pnpm wallets:status`:

- request Sepolia ETH from a faucet listed on the official
  [ethereum.org networks page](https://ethereum.org/developers/docs/networks/#sepolia);
- request EVM testnet CTC in Creditcoin's Discord `token-faucet` channel, following the official
  [Creditcoin faucet guide](https://docs.creditcoin.org/wallets/using-testnet-faucet).

Re-run `pnpm wallets:status` and require `readyForDeployment: true`. Faucet forms and Discord receive
the public EVM address only. Never paste a private key, seed phrase, `.env` content, or wallet export.

## Recommended automated vertical slice

Once both native balances are nonzero, run the single resumable command:

```bash
pnpm testnet:gate2
```

It performs the two deployments below, then mints/approves MockUSDC, opens a funded intent, builds and
device-signs a deterministic receipt, submits `finalizeSession`, waits for live attestation, generates
and preflight-verifies the proof, settles, and broadcasts the same proof again with explicit gas. The
second target receipt must be reverted. Progress lives in ignored `worker-state/gate2.json`; rerunning
the same command resumes rather than creating a second source session. A completed run atomically
writes public facts to `deployments/gate2-evidence.json`.

The numbered steps below remain available for manual diagnosis and dashboard demonstrations.

## 1. Deploy Sepolia source registry

If burners were not generated by the helper, copy `.env.example` to an ignored root `.env` and
provide these local variables:
